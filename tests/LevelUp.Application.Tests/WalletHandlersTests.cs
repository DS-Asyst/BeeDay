...
public sealed class WalletHandlersTests
{
    ...
    public async Task DeleteTag_RemovesAssociationAndKeepsTransaction()
    {
        var repo = new Repo();
        repo.CreateCurrentUser();
        var tagId = await new CreateWalletTagCommandHandler(repo).Handle(new(new("Food", "#123456")), TestContext.Current.CancellationToken);
        var transactionId = await new CreateTransactionCommandHandler(repo).Handle(
            ...
            TestContext.Current.CancellationToken);
        await new DeleteWalletTagCommandHandler(repo).Handle(new(tagId), TestContext.Current.CancellationToken);
    ...
    public async Task UpdateTag_RejectsDuplicateNameForCurrentUser()
    {
        var repo = new Repo();
        repo.CreateCurrentUser();
        var first = await new CreateWalletTagCommandHandler(repo).Handle(new(new("Food", null)), TestContext.Current.CancellationToken);
        var second = await new CreateWalletTagCommandHandler(repo).Handle(new(new("Transport", null)), TestContext.Current.CancellationToken);
        await Assert.ThrowsAsync<InvalidDomainStateException>(() => new UpdateWalletTagCommandHandler(repo).Handle(
