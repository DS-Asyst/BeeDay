...
{
    [Fact] public async Task CreateCharacterHandler_SeparatesUserAndCharacter() { var r = new Repo(); await new CreateCharacterCommandHandler(r).Handle(new CreateCharacterCommand(new("Tiago", "tiago", CharacterClass.Warrior)), TestContext.Current.CancellationToken); Assert.Equal("Tiago", r.Data.CurrentUser!.Name); Assert.Equal("tiago", r.Data.CurrentCharacter!.Nickname); Assert.Equal(r.Data.CurrentUser.Id, r.Data.CurrentCharacter.UserId); }
