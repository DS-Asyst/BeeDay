...
public sealed class LevelUpDbContextTests
{
    private const string TestConnectionString =
        "Server=(localdb)\\mssqllocaldb;Database=LevelUpDbContextTests;Trusted_Connection=True;TrustServerCertificate=True;";
    ...
    [Fact]
    public void AddLevelUpInfrastructure_ResolvesDbContextFactoryWithoutThrowing()
    {
        ...
        services.AddLevelUpInfrastructure(configuration);
        using var provider = services.BuildServiceProvider();

        var factory = provider.GetRequiredService<IDbContextFactory<LevelUpDbContext>>();
        using var context = factory.CreateDbContext();

        Assert.NotNull(context.Model);
    }

    private static LevelUpDbContext CreateContext()
    {
        var options = new DbContextOptionsBuilder<LevelUpDbContext>()
            .UseSqlServer(TestConnectionString)
            .Options;

        return new LevelUpDbContext(options);
    }
}
