...
public sealed class AccountSidePanelTests
{
    ...
    public void RendersInstitutionalBrandingInsteadOfSocialMedia()
    {
        ...
        Assert.Contains("BEE", cut.Find(".support-drawer__brand-level").TextContent, StringComparison.Ordinal);
        Assert.Contains("DAY", cut.Find(".support-drawer__brand-up").TextContent, StringComparison.Ordinal);
