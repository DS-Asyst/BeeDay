...
public sealed class WalletFiltersTests : BunitContext
{
    ...
    public async Task InvokesFilterCallbacks()
    {
        ...
        cut.Find(".wallet-secondary-filters select").Change("Income");
        cut.Find(".wallet-secondary-filters input[type='date']").Change("2026-07-01");
    ...
    {
        var cut = Render<WalletFilters>();

        var toggle = cut.Find(".wallet-filter-toggle");
        Assert.Equal("false", toggle.GetAttribute("aria-expanded"));

        var secondary = cut.Find("#wallet-secondary-filters");
        Assert.True(secondary.HasAttribute("hidden"));
        Assert.Empty(secondary.QuerySelectorAll("input, select, button"));
    }

    [Theory]
    [InlineData(nameof(WalletFilters.TypeFilter))]
    [InlineData(nameof(WalletFilters.StartDate))]
    [InlineData(nameof(WalletFilters.EndDate))]
    [InlineData(nameof(WalletFilters.Sort))]
    public void ToggleStartsExpandedWhenASecondaryFilterIsAlreadyActive(string activeParameter)
    {
        var cut = Render<WalletFilters>(parameters =>
        {
            switch (activeParameter)
            {
                case nameof(WalletFilters.TypeFilter):
                    parameters.Add(component => component.TypeFilter, "Expense");
                    break;
                case nameof(WalletFilters.StartDate):
                    parameters.Add(component => component.StartDate, new DateOnly(2026, 1, 1));
                    break;
                case nameof(WalletFilters.EndDate):
                    parameters.Add(component => component.EndDate, new DateOnly(2026, 1, 31));
                    break;
                case nameof(WalletFilters.Sort):
        ...
        Assert.Equal("true", cut.Find(".wallet-filter-toggle").GetAttribute("aria-expanded"));
        Assert.False(cut.Find("#wallet-secondary-filters").HasAttribute("hidden"));
    }
    ...
    public async Task ClickingTheToggleExpandsAndCollapsesTheSecondarySection()
    {
        ...
        Assert.Equal("true", cut.Find(".wallet-filter-toggle").GetAttribute("aria-expanded"));
        Assert.False(cut.Find("#wallet-secondary-filters").HasAttribute("hidden"));
        Assert.NotEmpty(cut.Find("#wallet-secondary-filters").QuerySelectorAll("select"));

        await cut.Find(".wallet-filter-toggle").ClickAsync();
        Assert.Equal("false", cut.Find(".wallet-filter-toggle").GetAttribute("aria-expanded"));
        Assert.True(cut.Find("#wallet-secondary-filters").HasAttribute("hidden"));
        Assert.Empty(cut.Find("#wallet-secondary-filters").QuerySelectorAll("select, input, button"));
    }
    ...
    public async Task ClearFiltersInvokesTheCallbackWhenClicked()
    {
        ...
        await cut.Find(".wallet-active-filters button").ClickAsync();
...
public sealed class TransactionListTests : BunitContext
{
    ...
    public void DisablesPaginationWhileBusy()
    {
        ...
        Assert.All(cut.FindAll(".wallet-pagination button"), button => Assert.True(button.HasAttribute("disabled")));
    }
    ...
    public void MarksThePanelEmptyOnlyWhenThereAreNoTransactions()
    {
        ...
        Assert.Contains("wallet-main-panel--empty", emptyCut.Find("section.wallet-main-panel").ClassList);
        ...
        Assert.DoesNotContain("wallet-main-panel--empty", populatedCut.Find("section.wallet-main-panel").ClassList);
    }
...
public sealed class WalletTagManagerTests : BunitContext
{
    ...
    public async Task ClickingTheTagRowInvokesOnEditWithThatTag()
    {
        ...
        WalletTagResponse? edited = null;
        var cut = Render<WalletTagManager>(parameters => parameters
            .Add(component => component.Tags, [tag])
            .Add(component => component.OnEdit, (WalletTagResponse value) => edited = value));
    ...
    public void ActivationKeyOnTheTagRowInvokesOnEdit(string key)
    {
        ...
        var cut = Render<WalletTagManager>(parameters => parameters
            .Add(component => component.Tags, [tag])
            .Add(component => component.OnEdit, (WalletTagResponse _) => editInvoked = true));
    ...
    public void TagRowRendersNoEditOrDeleteButtons()
    {
        ...
        Assert.Empty(cut.Find(".wallet-tag-item").QuerySelectorAll("button"));
    }
