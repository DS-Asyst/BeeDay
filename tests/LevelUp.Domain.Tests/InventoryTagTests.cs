...
public sealed class InventoryTagTests
{
    [Fact]
    public void Create_normalizes_name_and_color()
    {
        var tag = InventoryTag.Create(Guid.NewGuid(), "  Food   and drinks  ", "#7a4fcb");
    ...
    {
        var tag = InventoryTag.Create(Guid.NewGuid(), "Food");

        Assert.Equal(InventoryTag.DefaultColor, tag.Color);
    }
    ...
    public void Create_rejects_invalid_colors(string color)
    {
        if (color.Length == 0)
        {
            var tag = InventoryTag.Create(Guid.NewGuid(), "Food", color);
            Assert.Equal(InventoryTag.DefaultColor, tag.Color);
            return;
        }

        Assert.Throws<DomainValidationException>(() => InventoryTag.Create(Guid.NewGuid(), "Food", color));
    }
