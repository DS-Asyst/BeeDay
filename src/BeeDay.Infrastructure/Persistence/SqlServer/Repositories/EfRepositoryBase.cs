...
/// this call's <see cref="BeeDayDbContext"/> come from, and who is responsible for disposing it."
...
{
    private readonly IDbContextFactory<BeeDayDbContext>? contextFactory;
    private readonly BeeDayDbContext? sharedContext;

    protected EfRepositoryBase(IDbContextFactory<BeeDayDbContext> contextFactory)
    {
        this.contextFactory = contextFactory;
    }

    protected EfRepositoryBase(BeeDayDbContext sharedContext)
    {
...
/// <summary>
/// A <see cref="BeeDayDbContext"/> handed to a repository method, together with whether that method is
/// responsible for disposing it (own, per-call context) or not (shared context owned by
/// <see cref="EfUnitOfWork"/>).
/// </summary>
internal readonly struct DbContextLease(BeeDayDbContext context, bool ownsContext) : IAsyncDisposable
{
    public BeeDayDbContext Context { get; } = context;
