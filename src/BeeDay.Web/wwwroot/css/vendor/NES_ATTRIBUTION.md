...
BeeDay uses a small, curated, BeeDay-recolored subset of NES.css as the implementation source for its internal pixel adapter (`../pixel-nes.css`). NES.css is **not** loaded globally and is **not** a general dependency of the application — see `docs/design-system/foundations.md` for the adapter's scope and restrictions.
...
- **License**: MIT — Copyright (c) 2018 B.C.Rikko \<https://github.com/BcRikko\> (full text preserved in `nes-core.beeday-excerpt.css`)

## What this covers

`nes-core.beeday-excerpt.css` in this folder is a **provenance-only** reference copy of the exact upstream rule blocks BeeDay's pixel adapter was derived from. It is never linked from `App.razor` and is never loaded by the application — it exists solely so a future license/version audit can diff BeeDay's adapter against real upstream text.
...
The Bootstrap 4.1.3 Reboot/Normalize preamble; every bare-element selector (`html`, `body`, `a`, `button`, `input`, `select`, `textarea`, `table`, `th`, `td`, `label`, headings, lists, forms); the global `body,code,html,kbd,pre,samp{font-family:"Press Start 2P"}` rule; `.nes-container.is-dark` / `.is-centered` / `.is-right` / `.with-title`; `.nes-btn.is-success` / `.is-warning` / `.is-error` / `.is-disabled`; `.nes-btn`'s embedded `cursor: url(data:image/png;base64,...)` (BeeDay uses native browser cursors — the custom cursor pack, formerly `cursors.css`, was removed in Sprint 20.3); `.nes-btn input[type="file"]`; and every other NES.css component family (badge, dialog, balloon, icon, field, select, radio, checkbox, progress, list, and all other container/button variants). None of these have a real BeeDay consumer.
...
- The upstream `.nes-btn`'s `::after` inset-shadow "3D press" depth layer, and its `:hover`/`:focus`/`:active` states, were evaluated but **not** adopted: `BeeDayButton` already implements an equivalent, working 3D-press mechanic via its own `box-shadow` system (see `design-system.css`), and layering NES's competing technique on top would produce a duplicated/conflicting depth effect. Only the pixel-corner border-image outline was adopted for the button; every other button state (hover, active, focus, disabled, loading) is unchanged, existing `BeeDayButton` behavior.
- `:focus` styling uses BeeDay's own `var(--beeday-focus-ring)`, never NES's own focus color.
...
- `nes-core.beeday-excerpt.css` is a reference copy only. Do not link it from any page or component.
