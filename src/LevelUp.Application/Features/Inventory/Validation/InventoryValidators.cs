...
namespace LevelUp.Application.Features.Inventory.Validation;
...
public sealed class SaveTransactionRequestValidator : AbstractValidator<SaveTransactionRequest>
{
    public SaveTransactionRequestValidator()
    {
        ...
        RuleFor(x => x.InventoryTagId).Must(id => id is null || id != Guid.Empty)
            .WithMessage("Inventory tag identifier cannot be empty.");
        RuleFor(x => x.Notes).MaximumLength(Transaction.MaximumNotesLength);
    }
}

public sealed class SaveInventoryTagRequestValidator : AbstractValidator<SaveInventoryTagRequest>
{
    public SaveInventoryTagRequestValidator()
    {
        RuleFor(x => x.Name).NotEmpty().MaximumLength(InventoryTag.MaximumNameLength);
...
public sealed class CreateInventoryTagCommandValidator : AbstractValidator<CreateInventoryTagCommand>
{
    public CreateInventoryTagCommandValidator() => RuleFor(x => x.Request).SetValidator(new SaveInventoryTagRequestValidator());
}

public sealed class UpdateInventoryTagCommandValidator : AbstractValidator<UpdateInventoryTagCommand>
{
    public UpdateInventoryTagCommandValidator()
    {
        RuleFor(x => x.Id).NotEmpty();
        RuleFor(x => x.Request).SetValidator(new SaveInventoryTagRequestValidator());
    }
}

public sealed class DeleteInventoryTagCommandValidator : AbstractValidator<DeleteInventoryTagCommand>
{
    public DeleteInventoryTagCommandValidator() => RuleFor(x => x.Id).NotEmpty();
}
