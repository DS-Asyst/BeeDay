...
| `BeeDay.Web` | Composition root único (`Program.cs`), páginas/componentes Blazor Server, tradução de eventos de UI em `ISender.Send(...)` via a fachada `BeeDayWebService` | Grep por `"BeeDayDbContext"` em todo `src/BeeDay.Web` → 0 ocorrências — nenhum tipo EF Core concreto é referenciado fora de Infrastructure. |
