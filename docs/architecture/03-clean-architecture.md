...
| `BeeDay.Application` | Casos de uso via Commands/Queries MediatR, Requests/Responses, interfaces de repositório e serviços que `Infrastructure` implementa (contratos internos, não um projeto separado — ver [ADR-007](../adr/ADR-007-in-process-application-contracts.md)), validação (FluentValidation), pipeline de comportamentos | Zero `using Microsoft.EntityFrameworkCore`; referencia apenas `BeeDay.Domain` como `ProjectReference`. |
...
| `BeeDay.Web` | Composition root único (`Program.cs`), páginas/componentes Blazor Server, tradução de eventos de UI em `ISender.Send(...)` via a fachada `BeeDayWebService`, e camada responsável pela representação localizada (en-US/pt-BR) de toda mensagem de UI via `IStringLocalizer` — ver [`docs/web/07-localization.md`](../web/07-localization.md) | Grep por `"BeeDayDbContext"` em todo `src/BeeDay.Web` → 0 ocorrências — nenhum tipo EF Core concreto é referenciado fora de Infrastructure. Grep por `IStringLocalizer` fora de `BeeDay.Web` → 0 ocorrências. `ResourceManager` puro (não `IStringLocalizer`) é usado deliberadamente em `BeeDay.Infrastructure/Identity/IdentityEmailComposer.cs` para localizar e-mail transacional, sem depender de Web — exceção documentada, ver [ADR-006](../adr/ADR-006-transactional-email-localization-boundary.md) (`VALID`, Sprint 31.2) e [`docs/infrastructure/06-transactional-email.md`](../infrastructure/06-transactional-email.md). |
