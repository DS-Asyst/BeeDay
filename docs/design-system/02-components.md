...
| `BeeDayHero` | `Title`, `Eyebrow?`, `Subtitle?`, `Surface?` (`BeeDayPaletteToken`, restrito a `Cor0`/`Cor8` para o papel de page header — [`brand/03-color-palette.md`](../brand/03-color-palette.md)), illustration/action/support slots, `Variant`, `Compact`, `Class?`, attributes | 12 (InstitutionalPageShell × 11 rotas, ExperienceSystemHome) + Wallet | `Default`/`Onboarding`; illustration some no compacto; full-bleed quando é irmão direto de `.beeday-main` (não aninhado num `article`/`.page-content` capturado pelo reading-width de `polish.css`). `BeeDayHeroTests`, `InstitutionalPageShellTests`; `BeeDayHero.razor.css` |
