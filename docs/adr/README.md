...
|---|---|---|
| [`ADR-001-contract-first.md`](ADR-001-contract-first.md) | Adotar Contract-First em todas as fronteiras (adoção parcial em código — nenhum projeto Contracts separado foi criado). | `SUPERSEDED` quanto ao mecanismo, por [ADR-007](ADR-007-in-process-application-contracts.md) (2026-08-22) — o objetivo (Application depende de abstrações, nunca de Infrastructure) continua válido e é a realidade atual; o mecanismo literal decidido (projeto `Contracts` separado) foi formalmente substituído pela forma real implementada (interfaces dentro de `BeeDay.Application`), agora documentada pelo ADR-007. Nota de supersessão aplicada ao Status do ADR-001, corpo da decisão preservado sem reescrita. |
