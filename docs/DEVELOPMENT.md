...
Use `hmg` for integrated development and short-lived `feature/...` branches for changes. Open pull requests into `hmg`; promote validated releases from `hmg` to `prd`.
...
`.github/workflows/deploy-prd.yml` runs on push to `prd` or manual dispatch. It:
