...
| [`08-fast-pr-validation-decision.md`](08-fast-pr-validation-decision.md) | Registro de decisão da Sprint 19.4: por que `BeeDay CI` não podia ser renomeado para `BeeDay — Pull Request Validation` na época (dependências rastreadas até 19.7/19.8/19.8.4, ambas resolvidas — rename em si ainda `DEFERRED`, pendente de transição do Ruleset de `hmg`), decisão formal de manter E2E em toda PR, a remoção de `prd` do trigger `pull_request`, e o redesenho de `BeeDay CI` como Fast HMG Gate — Format/Infrastructure/Web/E2E movidos para `Release Quality Gate` com evidência remota real (Sprint 19.8.5) |
