...
| [`07-validation-matrix.md`](07-validation-matrix.md) | Matriz oficial `Validation × Stage` — inventário de todos os testes/validações do BeeDay, duração/criticidade/dependências/flakiness medidas, e classificação de estágio atual vs. recomendado — EPIC 19, Sprint 19.3. Entrada oficial para as Sprints 19.4-19.9; a recomendação de estágio para Infrastructure/Web/E2E foi substituída na Sprint 19.8.5 (ver `08-fast-pr-validation-decision.md` §12) |
| [`08-fast-pr-validation-decision.md`](08-fast-pr-validation-decision.md) | Registro de decisão da Sprint 19.4 sobre por que `BeeDay CI` não podia ser renomeado na época, o redesenho de `BeeDay CI` como Fast HMG Gate com evidência remota real (Sprint 19.8.5), e a finalização de identidade — rename para `BeeDay — Pull Request Validation` com descoberta empírica de check context e transição do Ruleset de `hmg` (Sprint 19.8.6) |
| [`09-pipeline-performance.md`](09-pipeline-performance.md) | Baseline de performance medido (6 execuções reais), achado confirmado de rebuild redundante em `dotnet publish` (11.3s eliminados), cache de NuGet e de browsers Playwright, candidatos de otimização rejeitados com justificativa (Sprint 19.5), e a redução do tempo de `BeeDay CI` de ~6m23s para uma faixa estimada de ~2m10s-2m20s via Fast HMG Gate (Sprint 19.8.5) — EPIC 19 |
...
8. `08-fast-pr-validation-decision.md` — por que o rename de `ci.yml` ficou bloqueado por várias
   Sprints, o redesenho como Fast HMG Gate, e a finalização de identidade em `BeeDay — Pull
   Request Validation` (Sprint 19.8.6).
